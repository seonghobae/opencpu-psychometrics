...
WORKDIR /app
RUN apt-get update -y && apt-get install aptitude wget unattended-upgrades bash -y && add-apt-repository -y ppa:opencpu/opencpu-2.2 && add-apt-repository -y "deb https://cloud.r-project.org/bin/linux/ubuntu $(lsb_release -cs)-cran40/" && unattended-upgrades
SHELL ["/bin/bash", "-l", "-c"]
