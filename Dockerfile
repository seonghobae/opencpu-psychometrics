...
WORKDIR /app
RUN apt-get update && \
    apt-get install -y unattended-upgrades ntpsec git wget lsb-release software-properties-common gnupg build-essential dirmngr libopenblas-dev && unattended-upgrades && \
...
RUN aptitude install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin libopenblas-dev libproj-dev libmagick++-dev libgfortran-~N-dev libgcc-~N-dev libglpk-dev libfftw3-dev libfftw3-mpi-dev libxslt1-dev libopenblas-dev libopenblas64-dev libarchive-dev libpoppler-cpp-dev libffmpeg-~N-dev libfontconfig1-dev libcurl4-openssl-dev poppler-cpp r-base r-base-dev opencpu-server rstudio-server cmake default-java libharfbuzz-dev libfribidi-dev libcairo2-dev && apt-get clean -y
