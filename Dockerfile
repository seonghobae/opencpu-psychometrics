...
RUN apt-get update && \
    ...
    apt-get clean -y && \
    R -e "options(timeout=10000); options(repos = 'https://cran.asia'); options(BioC_mirror = 'https://cran.asia', ask = F); install.packages('BiocManager, dependencies = TRUE, quiet = TRUE, Ncpus = parallel::detectCores()); BiocManager::install(ask = F, quiet = TRUE, Ncpus = parallel::detectCores()); BiocManager::install(available.packages()[,1], ask = F, dependencies = T, quiet = TRUE, Ncpus = parallel::detectCores())"
